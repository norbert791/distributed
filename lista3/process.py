...
class Process:
    ...
    def connect_to(self, other_process: 'Process') -> None:
    ...
    def send_message(self, to_pid: int, message: any, is_marker: bool = False) -> None:
        """Send a message to another process."""
        if to_pid in self.outgoing_channels:
            self.outgoing_channels[to_pid].send(message, is_marker)
    
    def receive_message(self, from_pid: int) -> Optional[Message]:
    ...
    def update_state(self, amount: int) -> None:
    ...
    def _handle_marker(self, from_pid: int) -> None:
